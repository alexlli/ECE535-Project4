...
class ResNetMapper(nn.Module):
    resnet = resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1).double()
...
def ur_fall_idxs_to_imgs(idxs):
    ...
    for idx_frame, frame in enumerate(idxs):
        ...
        f_img = os.path.join(f"download/ur_fall", is_fall, f"cam0-rgb", f"{is_fall}-{str(run).zfill(2)}-cam0-rgb",
