...
class FL:
    ...
    def start(self):
        ...
        while True:
            ...
            print("spliting train A")
            print(set(server_train_A["y"]))
            print(set(server_test["y"]))
            if set(server_train_A["y"]) == set(server_test["y"]):
                break
        while True:
            server_train_B = split_server_train(data_train, self.config)
            print("spliting train B")
            print(set(server_train_B["y"]))
            print(set(server_test["y"]))
        ...
        # per class result table: round, loss, accuracy, weighted_f1, metrics_dict)
        row = 0
            ...
            if t % self.eval_interval != 0:
                ...
                with torch.no_grad():
                    test_loss, test_accuracy, test_f1 = server.eval(
                        server_test)
                    loss, accuracy, weighted_f1, macro_f1, class_metrics = server.eval_per_class(data_test)
                result_table[row] = np.array(
                    (t+1, local_ae_loss, train_loss, train_accuracy, test_loss, test_accuracy, test_f1))
                #per_class_result_table[row] = np.concatenate(
                #   np.array([t+1, weighted_f1, macro_f1]), np.array(list(client_matrix.values())))
                row += 1
                self.write_result(result_table)
                self.write_per_class_result(row,  weighted_f1, class_metrics)
    ...
    def write_per_class_result(self, row, weighted_f1, client_matrix):
        """ Writes simulation results into a result.txt file

        Args:
            result_table: a 2-d numpy array contraining rows of per class simulation results
        """
        if self.is_mpi:
            results_path = os.path.join(self.results_path, f"rep_{self.rank}")
        else:
            results_path = self.results_path
        row = np.array(row).reshape(-1).astype(float)
        weighted_f1 = np.array(weighted_f1).reshape(-1).astype(float)
        client_matrix_values = np.array(list(client_matrix.values()))

        # Flatten the client_matrix_values to concatenate into a 1D array
        client_matrix_flattened = client_matrix_values.flatten()

        result_table = np.concatenate([row, weighted_f1, client_matrix_flattened])
        Path(results_path).mkdir(parents=True, exist_ok=True)
        np.savetxt(os.path.join(results_path, "per_class_results.txt"),
                   result_table, delimiter=",", fmt='%s')