    ...
    for dataset in DATASETS:
        for ae in aes[dataset]:
            for modalities in combo[dataset]:
                ...
                legends = {"client_A_label_A_test_A": r"$\mathregular{UmFL_A}$",
                           "client_B_label_B_test_B": r"$\mathregular{UmFL_B}$",
                           "client_AB_label_AB_test_A": r"$\mathregular{MmFL_{AB}-L_{AB}-T_A}$",
                           "client_AB_label_AB_test_B": r"$\mathregular{MmFL_{AB}-L_{AB}-T_B}$", }
                for k in schemes:
...
#                        ), ae, f"{modalities[0]}_{modalities[1]}", schemes[k], "results.txt")
                ...
                metric_label = "accuracy" if METRIC == "accuracy" else r"$\mathregular{F_1}$"
                ax.set_ylabel(
    ...
    for dataset in DATASETS:
        for ae in aes[dataset]:
            for modalities in combo[dataset]:
                ...
                legends = {
                    "client_A_label_A_test_A": r"$\mathregular{UmFL_A}$",
                    "client_AB_label_B_test_A": r"$\mathregular{MmFL_{AB}-L_B-T_A}$",
                    "client_ABA_label_B_test_A": r"$\mathregular{MmFL_{ABA}-L_B-T_A}$",
                    "client_ABB_label_B_test_A": r"$\mathregular{MmFL_{ABB}-L_B-T_A}$",
                    "client_ABAB_label_B_test_A": r"$\mathregular{MmFL_{ABAB}-L_B-T_A}$",
                    "ablation_label_B_test_A": r"$\mathregular{Abl-L_B-T_A}$",
                    "client_B_label_B_test_B": r"$\mathregular{UmFL_B}$",
                    "client_AB_label_A_test_B": r"$\mathregular{MmFL_{AB}-L_A-T_B}$",
                    "client_ABB_label_A_test_B": r"$\mathregular{MmFL_{ABB}-L_A-T_B}$",
                    "client_ABA_label_A_test_B": r"$\mathregular{MmFL_{ABA}-L_A-T_B}$",
                    "client_ABAB_label_A_test_B": r"$\mathregular{MmFL_{ABAB}-L_A-T_B}$",
                    "ablation_label_A_test_B": r"$\mathregular{Abl-L_A-T_B}$",
                }
                        ...
                        for rep in range(N_REPS):
                            if "ablation" not in k:
                                rep_file = os.path.join(
                                    "results", dataset.lower(), ae, f"{modalities[0]}_{modalities[1]}", schemes[k][1],
                                    f"rep_{rep}",
                                    "results.txt")
                            else:
                                rep_file = os.path.join(
                                    "results", dataset.lower(), "ablation", f"{modalities[0]}_{modalities[1]}", schemes[k][1],
                                    f"rep_{rep}",
                                    "results.txt")
                            data = np.loadtxt(rep_file, delimiter=",")
                            x_all = data[:, 0]
