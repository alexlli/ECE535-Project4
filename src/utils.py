...
def rebalance_opp_data(data):
    ...
    # must include unlabeled data since the test data includes unlabeled. The labels will not 
    # match during splitting if the training data does not include unlabeled data but testing data does
...
def load_data(config):
    ...
    if data == "opp":
        ...
        data_train = balance_multimodal_data_exclude(data_train)
        ...
        data_train = balance_multimodal_data_smote(data_train)
        return (data_train,  data_test)

def balance_multimodal_data_smote(data_train):
    """
    Balance multimodal time series data using SMOTE
    """
    #print ("plot label before balance")
    #plot_class_distribution(data_train["y"], "Label distribution")


    # Flatten/reshape the time series data
    X_A = data_train["A"].reshape(len(data_train["y"]), -1)
    X_B = data_train["B"].reshape(len(data_train["y"]), -1)
    
    # Combine modalities horizontally
    ...
    split_point = X_A.shape[1]
    X_A_resampled = X_resampled[:, :split_point]
    X_B_resampled = X_resampled[:, split_point:]
    
    # Reshape back to original dimensions
    A_balanced = X_A_resampled.reshape(-1, data_train["A"].shape[1])
    B_balanced = X_B_resampled.reshape(-1, data_train["B"].shape[1])

    #print ("plot labels after rebalance")
    #plot_class_distribution(y_resampled, "Balanced label distribution")
...
def balance_multimodal_data_exclude(data_train, exclude_value=0):
    """
    Balance multimodal time series data while excluding specific label value
    
    Args:
        data_train: Dictionary containing A, B modalities and labels y
        exclude_value: Label value to exclude from balancing (e.g., 0 for unlabeled)
        method: "over" for oversampling, "under" for undersampling
    """
   

    # Get original shapes
    orig_shape_A = data_train["A"].shape
    orig_shape_B = data_train["B"].shape
    
    # Split data into excluded and included
    mask_include = data_train["y"] != exclude_value
    mask_exclude = data_train["y"] == exclude_value
    
    # Prepare data for balancing (only non-excluded data)
    X_A_to_balance = data_train["A"][mask_include].reshape(sum(mask_include), -1)
    X_B_to_balance = data_train["B"][mask_include].reshape(sum(mask_include), -1)
    y_to_balance = data_train["y"][mask_include]
    
    print ("plot label before balance")
    plot_class_distribution(y_to_balance, "Label distribution")

    # Store excluded data
    X_A_excluded = data_train["A"][mask_exclude]
    X_B_excluded = data_train["B"][mask_exclude]
    y_excluded = data_train["y"][mask_exclude]
    
    # Combine modalities for balancing
    X_combined = np.hstack([X_A_to_balance, X_B_to_balance])
    
    # Apply SMOTE
    smote = SMOTE(random_state=42)
    X_resampled, y_resampled = smote.fit_resample(X_combined, y_to_balance)

    print ("plot labels after rebalance")
    plot_class_distribution(y_resampled, "Balanced label distribution")

     # Split back into modalities
    split_point = orig_shape_A[1]
    X_A_balanced = X_resampled[:, :split_point]
    X_B_balanced = X_resampled[:, split_point:]
    
    # Recombine with excluded data
    A_final = np.vstack([X_A_excluded, X_A_balanced])
    B_final = np.vstack([X_B_excluded, X_B_balanced])
    y_final = np.concatenate([y_excluded, y_resampled])
...
def download_UR_fall():
    ...
    for i in range(1, 31):
        ...
        directory = 'download/UR_FALL/fall/cam0-d'
        ...
        directory = 'download/UR_FALL/fall/cam1-d'
        if not os.path.exists(directory):
            os.makedirs(directory)
        ...
        directory = 'download/UR_FALL/fall/cam0-rgb'
        if not os.path.exists(directory):
            os.makedirs(directory)                    
        ...
        directory = 'download/UR_FALL/fall/cam1-rgb'
        if not os.path.exists(directory):
            os.makedirs(directory)                    
        ...
        directory = 'download/UR_FALL/fall/sync'
        if not os.path.exists(directory):
            os.makedirs(directory)
        ...
        directory = 'download/UR_FALL/fall/acc'
        if not os.path.exists(directory):
            os.makedirs(directory)
    ...
    for i in range(1, 41):
        ...
        directory = 'download/UR_FALL/adl/cam0-d'
        if not os.path.exists(directory):
            os.makedirs(directory)
        ...
        directory = 'download/UR_FALL/adl/cam0-rgb'
        if not os.path.exists(directory):
            os.makedirs(directory)
        ...
        directory = 'download/UR_FALL/adl/sync'
        if not os.path.exists(directory):
            os.makedirs(directory)
        r = requests.get(sync_file)
        open(
        ...
        directory = 'download/UR_FALL/adl/acc'
        if not os.path.exists(directory):
            os.makedirs(directory)
        r = requests.get(acc_file)
        open(
    ...
    directory = 'download/UR_FALL/fall'
    if not os.path.exists(directory):
            os.makedirs(directory)
    ...
    directory = 'download/UR_FALL/adl'
    if not os.path.exists(directory):
            os.makedirs(directory)
